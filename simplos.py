...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Simplos - A (too) simple OS."
    )
    parser.add_argument(
        "--baud", type=int, nargs=1, required=True, help="BAUD rate"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "--build-docker", action="store_true", help="Rebuild docker container."
    )
    group.add_argument(
        "-f",
        "--flash",
    ...
    group.add_argument(
        "-c", "--clean", action="store_true", help="Remove build files."
    )
...
def main(baud, build_docker, flash, build, simulate, debug, clean):
    ...
    avr_docker = (
        f"sudo docker run -it --rm --mount type=bind,"
        f"source={src_dir_path.resolve()},target=/build"
        " avr_docker"
    )
    ...
    if debug:
        commands += [
            (
                'avr-gdb src/simplos.out -ex "target'
                'remote :1234" -ex "directory src/"'
            )
        ]
    if clean:
